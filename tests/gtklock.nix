...
let
  ...
  checks = [
    ...
    {
      name = "enabled: the lock service is Type=notify so a suspend waits for the actual lock, not the fork";
      ok =
        let
          sc = enabled.config.systemd.user.services.gtklock-session-lock.serviceConfig;
        in
        sc.Type == "notify" && sc.NotifyAccess == "all";
    }
    {
      name = "enabled: gtklock's lock-command signals readiness (on-lock) when the screen actually locks";
      ok = lib.hasInfix "on-lock" enabled.config.programs.gtklock.config.main.lock-command;
    }
  ];
