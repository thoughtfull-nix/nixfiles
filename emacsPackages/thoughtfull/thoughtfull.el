;;; thoughtfull.el --- Configure Emacs generally                         -*- lexical-binding: t; -*-
...
;; Automatic theme switching based on gtk color-scheme
(use-package auto-dark
  :custom
  (auto-dark-themes '((adwaita-dark) (adwaita)))
  :config
  (auto-dark-mode t)
  (add-hook 'after-make-frame-functions #'tfl-server-auto-dark))

(defun tfl-server-auto-dark (frame)
  ;; Emacs server cannot properly listen for light/dark theme changes if it loses its connection to
  ;; dbus (for example, if I log out), so re-enable auto-dark-mode on every emacsclient connection.
